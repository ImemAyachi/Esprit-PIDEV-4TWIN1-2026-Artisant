...
const AdminUsers = () => {
  ...
  const handleDelete = (id, name) => {
    setConfirmConfig({
      ...
      message: `Le compte de "${name}" sera définitivement supprimé. Cette opération est irréversible.`,
      confirmLabel: 'Supprimer',
      onConfirm: async () => {
        try { await api.delete(`/admin/users/${id}`); toast.success('Utilisateur supprimé'); load(); } catch (_) { toast.error('Erreur'); }
      }
    });
  };
                ...
                <tr key={u._id}>
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                      ...
                      {u.role !== 'SuperAdmin' && (
                        <button className="btn btn-sm btn-danger" onClick={() => handleDelete(u._id, `${u.firstName} ${u.lastName}`)}>Supprimer</button>
                      )}
                    </div>
                  </td>
                </tr>
