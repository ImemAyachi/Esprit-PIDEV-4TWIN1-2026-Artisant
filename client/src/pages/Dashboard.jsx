...
const Dashboard = () => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'Access Logs': return <AccessLogsPanel />;
    ...
    return (
        <div className="h-screen w-full bg-brand-cream flex overflow-hidden font-outfit">
            ...
            <aside className={`bg-brand-teal text-white border-r-8 border-brand-teal transition-all duration-300 flex flex-col ${sidebarOpen ? 'w-72' : 'w-20'}`}>
                ...
                {/* Role badge */}
                {sidebarOpen && (
                    <div className="mx-4 mt-4 px-3 py-2 bg-white/10 border border-white/20 flex items-center gap-2">
                        {role === 'artisan' && <Hammer size={12} className="text-brand-orange" />}
                        {role === 'manufacturer' && <Wrench size={12} className="text-brand-orange" />}
                        {role === 'expert' && <Leaf size={12} className="text-brand-orange" />}
                        <span className="text-[9px] font-black uppercase tracking-widest text-white/60 capitalize">{role}</span>
                    </div>
                )}
